...
export default function Users(props) {
  ...
  const handleArrayUser = arrUsers.filter((item) => item?.soDt?.length === 11 && !item?.soDt?.match(/^[a-zA-Z\s]*$/));
  const dataArrUser = handleArrayUser.map((user, index) => {
    if (user && user.soDt) {
      return {
        key: index,
    ...
    },
    {
      ...
      render: (text, account) => {
        return (
          <>
            <NavLink
              to={`/admin/detail-user/${account.taiKhoan}`}
              key={0}
            ...
            <button
              ...
              onClick={() => {
                if (
                  window.confirm(
                    "Bạn có chắc muốn xóa tài khoản" + " " + account.taiKhoan
                  )
                ) {
